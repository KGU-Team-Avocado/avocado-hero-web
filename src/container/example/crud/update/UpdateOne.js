...
export default () => {
    ...
    const handleButton = () => {
        if (key != '') {
            // 데이터 받아오는 작업 시작
            axios.get('/testsRouter/modify/' + key)
                .then((response) => {
                    console.log(JSON.stringify(response.data))
                    setModifiedValue(JSON.stringify(response.data));
    ...
    return (
        <>
            <h4>특정 아이디값 수정하기 (위의 전체 데이터에서 test_id를 기반으로 검색해보세요)</h4>
            ...
            <div>검색 결과 : {modifiedValue == 'null' ? '검색된 데이터가 없습니다.' : modifiedValue}</div>
        </>
    )
}